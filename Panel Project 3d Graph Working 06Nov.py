...
ax3d.set_xlim(-1, cols +1)
ax3d.set_ylim(-1, (len(regions) // cols) +1)
...
for i, r in enumerate(regions):
    ax3d.text(xpos[i] +width[i] / 2, ypos[i] + depth[i]/2, height[i]+(0.05*max(height)),r, fontsize = 8, zdir='x', zorder =10)
...
def update(val):
    ...
    for i, r in enumerate(regions):
        ax3d.text(xpos[i] +width[i] / 2, ypos[i] + depth[i]/2, new_height[i]+(0.05*max(new_height)),r, fontsize = 8, zdir='x', zorder =10)
