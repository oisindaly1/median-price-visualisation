...
fig_go.update_layout(scene=dict(aspectmode="manual", aspectratio=dict(x=1, y=1, z=0.8), 
...
for region_name in regions:
    ...
    trace = go.Mesh3d(x=xv,y=yv,z=zv, i=i,j=j,k=k, intensity = [height]*len(i), 
                      customdata=[[height, date_str]]*len(i),colorscale = "Viridis", cmin = h_min, cmax = h_max,
...
for d in dates:
    ...
    frames.append(go.Frame(data=frame_data, name = str(d), layout=dict(title=dict(text=f"3D Visualisation of Median Price by Region - {d.strftime('%B %Y')}"))))
